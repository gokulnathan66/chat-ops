resource "aws_cloudwatch_event_rule" "eval_runner_schedule" {
  ...
  description         = "Trigger eval_runner Lambda to process stale sessions"
...
resource "aws_cloudwatch_event_rule" "golden_runner_schedule" {
  ...
  description         = "Trigger golden_dataset_runner to test RAG quality against golden Q&A"
...
resource "aws_cloudwatch_event_rule" "pca_runner_schedule" {
  ...
  description         = "Trigger pca_runner every 15 minutes for post-conversation emotion and topic analysis"
