...
resource "aws_cloudwatch_event_rule" "golden_dataset_runner" {
  name                = "${var.project}-${var.env}-golden-dataset-runner"
  description         = "Trigger golden dataset runner on schedule"
  schedule_expression = var.golden_schedule
...
resource "aws_cloudwatch_event_target" "golden_dataset_runner" {
  rule      = aws_cloudwatch_event_rule.golden_dataset_runner.name
  target_id = "golden-dataset-runner"
  arn       = aws_lambda_function.golden_dataset_runner.arn
}

resource "aws_lambda_permission" "eventbridge_golden_runner" {
  statement_id  = "AllowEventBridgeInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.golden_dataset_runner.function_name
  principal     = "events.amazonaws.com"
  source_arn    = aws_cloudwatch_event_rule.golden_dataset_runner.arn
}
